...
export default function NuevoServicio() {
    ...
    try {
      const r = await fetch(`${CP_API}/api/servicios/contratos`, {
        ...
        body: JSON.stringify({
          ...
          cantidad_disponible:  form.cantidad_disponible  ? parseFloat(form.cantidad_disponible)  : null,
  ...
  const mostrarModalidad  = form.tipo === 'contrato_horas'
  const mostrarCantidad   = form.tipo === 'contrato_visitas' ||
                            (form.tipo === 'contrato_horas' && form.modalidad === 'prepago')
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-[#E8EAEC] p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              ...
              maxLength={4}
              placeholder="ESPH"
              className="mt-1 w-full border border-[#E8EAEC] rounded-lg px-3 py-2 text-sm font-mono uppercase"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <select value={form.tipo} onChange={e => { set('tipo', e.target.value); set('modalidad', '') }}
            className="mt-1 w-full border border-[#E8EAEC] rounded-lg px-3 py-2 text-sm">
        ...
        {mostrarModalidad && (
          <div>
            ...
            <select value={form.modalidad} onChange={e => set('modalidad', e.target.value)}
              className="mt-1 w-full border border-[#E8EAEC] rounded-lg px-3 py-2 text-sm">
              <option value="">Seleccionar...</option>
              <option value="prepago">Prepago (horas fijas contratadas)</option>
              <option value="bajo_demanda">Bajo demanda (sin tope, se factura lo consumido)</option>
            </select>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-[#5f6b75]">
                {form.tipo === 'contrato_visitas' ? 'Visitas contratadas' : 'Horas contratadas'} *
              </label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="date" lang="es-CR" value={form.fecha_inicio} onChange={e => set('fecha_inicio', e.target.value)}
              className="mt-1 w-full border border-[#E8EAEC] rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-sm text-[#5f6b75]">Fecha fin (opcional)</label>
            <input type="date" lang="es-CR" value={form.fecha_fin} onChange={e => set('fecha_fin', e.target.value)}
              className="mt-1 w-full border border-[#E8EAEC] rounded-lg px-3 py-2 text-sm" />
          </div>
        </div>
